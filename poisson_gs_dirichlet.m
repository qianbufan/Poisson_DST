function [x, y, U, info] = poisson_gs_dirichlet(a, b, I, J, f_handle, alpha_handle, opts)
...
for iter = 1:opts.maxIter
    ...
    for i = 2:I+1
        for j = 2:J+1
            U(i, j) = ((U(i - 1, j) + U(i + 1, j)) / h^2 + (U(i, j - 1) + U(i, j + 1)) / k^2 + F(i - 1, j - 1)) / den;
        end
    end
    tmp = U(2:end-1, 2:end-1) - U_old(2:end-1, 2:end-1);
